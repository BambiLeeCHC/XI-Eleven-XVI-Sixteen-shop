  ...
  const run = (c: Cmd) => {
    ...
    try {
      if (c.kind === "exec") {
        ...
        // Firefox requires the tag name wrapped in angle brackets
        // ("<p>") — passing it bare (as Chrome/Safari happily accept)
        // makes Firefox silently no-op, which is why heading/paragraph/
        // quote buttons could appear "broken" depending on the browser.
      ...
      // execCommand throws in some locked-down/embedded browser contexts
      // instead of just returning false — don't let that crash the editor.
    }
    emit();
  };
