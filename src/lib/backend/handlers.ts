/**
 * Supabase implementations of every backend function the storefront calls.
 *
 * Reads and ordinary writes go straight to Postgres through row-level security.
 * Anything privileged — creating orders, talking to Stripe, Printful or the
 * email provider — goes through a Vercel API route holding the service key, so
 * the browser is never trusted with order state or a secret.
 */
...
/** Call one of the privileged API routes with the caller's access token. */
...
export const handlers: Record<string, Handler> = {
  ...
  "cart.getItems": async ({ sessionId }) => {
    ...
    return (rows ?? [])
      ...
      .map(row => ({ ...mapCartItem(row), product: mapProduct(row.product) }));
  },
  ...
  "cart.addItem": async ({ sessionId, productId, size, color, quantity }) => {
    const userId = await currentUserId();
    // Same product + size + colour is one cart line: two tees in the same size
    // but different colourways are different Printful variants.
    const existing = (
      unwrap(
        await supabase
          .from("cart_items")
  ...
  // Orders are written only by the server, which re-prices them from the
  // database. The browser supplies the address and quantities, nothing else.
  ...
  "orders.listBySession": async ({ sessionId }) => {
    if (!sessionId) return [];
    const rows = unwrap(
      await supabase.rpc("orders_for_session", { p_session_id: sessionId }),
    );
    return (rows ?? []).map(mapOrder);
  },
  ...
  "orders.getByStripeSession": async ({ stripeCheckoutSessionId }) => {
    const rows = unwrap(
      await supabase.rpc("order_for_stripe_session", {
        p_stripe_session_id: stripeCheckoutSessionId,
      }),
    ) as Row[];
  ...
  // Only an admin can move an order along by hand; for a customer this is a
  // no-op, because payment state is owned by the Stripe webhook. The checkout
  // page still calls it after redirecting, which is harmless.
    ...
    const row = unwrap(
      await supabase
        ...
        .insert({
          ...
          cover_image: args.coverImage ?? null,
          ...
          updated_at: now,
          read_minutes: estimateReadMinutes(args.content ?? ""),
  ...
  "blog.updatePost": async ({ postId, ...rest }) => {
    ...
    if (rest.content !== undefined)
      patch.read_minutes = estimateReadMinutes(rest.content);
    if (
    ...
    for (const seed of SEED_POSTS) {
      ...
      unwrap(
        await supabase.from("blog_posts").insert({
          ...
          cover_image: seed.coverImage ?? null,
  ...
  "shipping.getAll": async () => {
    const rows = unwrap(
      await supabase.from("shipping_settings").select("*"),
    ) as Row[];
  ...
  "tax.calculateTax": async ({ countryCode, stateCode, subtotalCents }) => {
    ...
    // Region type is always qualified: 'CA' is both California and Canada.
    const query =
  ...
  "tax.seedDefaults": async () => {
    // The full default table now ships as a database migration, so there is
    // nothing left for this button to insert.
  ...
  "admin.getOrder": async ({ orderId }) =>
    handlers["orders.getById"]({ orderId }),

  "admin.updateOrderStatus": async ({
    orderId,
    status,
    trackingUrl,
    trackingNumber,
  ...
  "admin.updateProduct": async ({ productId, ...fields }) => {
    if (
      ...
      throw new Error(
        "A product rotation must contain exactly eight ordered frames.",
      );
    }
  ...
  "admin.listCustomers": async () => {
    ...
    const orders = unwrap(
      await supabase.from("orders").select("user_id, total, created_at"),
    ) as Row[];
    ...
    return (profiles ?? []).map(profile => {
      ...
      return {
        ...
        lastOrderDate:
          own.length > 0 ? new Date(own[0].created_at).getTime() : null,
      };
    });
  },
  ...
  "admin.dashboardStats": async () => {
    const orders = (
      (unwrap(await supabase.from("orders").select("*")) as Row[]) ?? []
    ).map(mapOrder) as any[];
    const products = (
      (unwrap(await supabase.from("products").select("*")) as Row[]) ?? []
    ).map(mapProduct) as any[];
