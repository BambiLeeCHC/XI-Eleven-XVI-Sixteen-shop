...
function buildUserPrompt(
  ...
  const personalization =
    name || situation
      ? `\n\nThis reading is for ${name || "the reader"}. What they told us is going on right now: "${situation || "not specified"}". Weave this in per the personalization rule.`
      : "";
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    const { spread, name, situation } = (req.body ?? {}) as {
      spread?: SpreadCardInput[];
      name?: string;
      situation?: string;
    };
