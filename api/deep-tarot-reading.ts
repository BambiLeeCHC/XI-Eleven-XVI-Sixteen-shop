...
function fallbackLongRead(
  name: string,
  situation: string,
  spread: SpreadCardInput[],
): string {
  const who = name || "friend";
  const sit = situation && situation !== "not specified" ? situation : "what you are carrying today";
  const cards = spread.map((c) => {
    const meaning = c.reversed ? c.card.reversed : c.card.upright;
    return `In the ${c.slotName} of this, **${c.card.name}**${c.reversed ? " (reversed)" : ""} speaks to ${sit}. ${meaning} The charge here is usable — not a warning, a next move.`;
  });
  return `${who}, the Long Read opens on ${sit}.\n\n${cards.join("\n\n")}\n\nThe throughline is already in your favor: the cards name support you can spend, not a problem you have to survive. Take one concrete step tonight that matches the Best Move card — something small enough to finish before you sleep.`;
}
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (!result.success) {
      ...
      console.error("Long Read Groq failed, using card-native fallback:", groqErrorMessage(failure));
      readingText = fallbackLongRead(name, situationText, spread);
      usedFallback = true;
    } else {
    ...
    if (user.email) {
      const emailResult = await sendResendEmail({
        ...
        html: buildKeepsakeEmailHtml({
          name,
          readingHtml: readingTextToHtml(readingText),
        }),
        text: readingText,
      });
