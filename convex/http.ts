...
http.route({
  path: "/admin/upsert-product",
  method: "POST",
  handler: httpAction(async (ctx, request) => {
    try {
      const authHeader = request.headers.get("Authorization");
      if (authHeader !== `Bearer ${process.env.VIKTOR_SPACES_PROJECT_SECRET}`) {
        return new Response("Unauthorized", { status: 401 });
      }
      const body = await request.json();
      const result = await ctx.runMutation("products:upsertFromPrintful" as any, body);
